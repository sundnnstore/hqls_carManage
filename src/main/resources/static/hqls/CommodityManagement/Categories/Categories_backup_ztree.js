...
function onBodyDown(event) {
	if (treeTemp == 'model') {
		if (!(event.target.id == "menuBtn" || event.target.id == "modelContent" || $(
				event.target).parents("#modelContent").length > 0)) {
			hideMenu();
		} else {
...
function beforeClick(treeId, treeNode) {
	...
	if (treeNode != undefined) {
		...
		parentTrs += '<tr>'
				...
				+ pname
				+ '</td>'
				+ '<td><button id="view" data-method="view" class="layui-btn view_c">添加子集</button>'
				+ '<button id="edit" data-method="edit" class="layui-btn layui-btn-normal edit_c">添加同级</button>'
				+ '<button id="edit" data-method="edit" class="layui-btn layui-btn-normal edit_c">修改</button>'
				+ '<button id="edit" data-method="edit" class="layui-btn layui-btn-normal edit_c">删除</button>'
				+ '</td></tr>';
